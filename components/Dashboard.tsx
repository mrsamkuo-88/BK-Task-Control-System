  ...
  const handleCreateAssignment = async () => {
      ...
      selectedVendorIds.forEach(vendorId => {
          const newTask: VendorTask = {
              ...
              requiredActions: ['確認時間地點', '準備器材/食材'],
              lastRemindedAt: null,
    ...
    <div className="min-h-screen bg-slate-50 flex font-sans text-slate-900">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-slate-300 transition-transform duration-300 ease-in-out transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static lg:inset-0 shadow-2xl flex flex-col`}>
        ...
        <div className="h-16 flex items-center px-6 border-b border-slate-800 bg-slate-950/50">
           ...
           <div>
               <h1 className="text-white font-bold text-lg tracking-tight">街趣防護</h1>
                          ...
                          <div>
                              ...
                              <div className="flex gap-2 overflow-x-auto pb-2">
                                  {vendorForm.serviceImages?.map((img, i) => (
                                      <div key={i} className="relative w-20 h-20 shrink-0 rounded-lg bg-slate-100 bg-cover bg-center border border-slate-200" style={{backgroundImage: `url(${img})`}}>
                                          <button 
                                              onClick={() => {
                                                  const newImages = vendorForm.serviceImages?.filter((_, idx) => idx !== i);
                                                  setVendorForm({...vendorForm, serviceImages: newImages});
                                              }}
                                              className="absolute -top-1 -right-1 bg-white rounded-full p-0.5 text-red-500 shadow-sm border border-slate-100 hover:bg-red-50"
                                          >
                                              <X size={12}/>
                                          </button>
                                      </div>
                                  ...
                                  <button 
                                      ...
                                      onClick={() => {
                                          ...
                                          if (url) {
                                              setVendorForm({...vendorForm, serviceImages: [...(vendorForm.serviceImages || []), url]});
                                          }
                                      }}
                                  >
                                      <Plus size={20}/>
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-8">
                       ...
                       <div>
                           <div className="flex items-center justify-between mb-3">
                               ...
                               {isEditing && <button onClick={handleAddActionItem} className="text-xs bg-emerald-50 text-emerald-600 px-2 py-1 rounded font-bold hover:bg-emerald-100">新增項目</button>}
                           </div>
